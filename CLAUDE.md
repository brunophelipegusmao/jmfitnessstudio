...
applies-to: Next.js | Tailwind CSS | shadcn/ui | BetterAuth | Drizzle ORM | NestJS | Fastify | Stripe | Zod | React | React Hook Form | react-number-format | Zustand | Vitest | Jest | Winston | TypeScript | @nestjs/schedule
...
social-providers: Google OAuth — disponível apenas para customer e student
...
student: apps/web (student) — based on active plan
customer: apps/web (ecommerce) only — no studio access

plan-access:
free: dashboard showing plan status only — no studio access
...
role-promotion:
customer → student: automatic via Stripe webhook on plan payment confirmed
...
invoice.payment_failed:
  notifica aluno — não altera role
customer.subscription.deleted:
  inicia grace: subscription_status=grace, grace_period_until=now+5days
customer.subscription.updated:
  cancel_at_period_end=true → subscription_status=canceling

SCHEMA-FIELDS (packages/db — users table):
...
DOMAIN-RULES:
checkin:
normal: student self-checkin (master plan, active ou grace)
...
financials:
plan_expires_at: managed by Stripe webhook — never set manually
payment_method: stripe (subscription) | manual (courtesy only)
stripe: webhook invoice.payment_succeeded → update plan_expires_at + role
manual: staff sets confirmed_by for courtesy cases only
stripe_payment_id: populated on stripe payments only
idempotency: check stripe_payment_id exists before updating

scheduler:
cron: daily at 02:00 — grace period processor
action: query users WHERE subscription_status='grace' AND grace_period_until < now()
result: role=customer, plan=free, subscription_status=expired, grace_period_until=null
logging: log every execution (start, users processed, errors)
...
customer-access: any registered user (role=customer or higher) can browse

NESTJS-PATTERNS:
http-adapter: Fastify only — never platform-express in production
structure: module per domain (auth, users, checkin, metrics, events, financials, products, scheduler)
env: FRONTEND_URL added to api .env for CORS
...
pattern: .test.tsx co-located with source
shared-rules:
...
pr-ready: types pass | no `any` | no duplicated logic | kebab-case files | all installs via CLI | tests written | swagger documented | logs added


packages/db/schema/ → all Drizzle table definitions
packages/validators/ → shared Zod schemas
apps/api/src/auth/ → BetterAuth guards and controller
apps/api/src/scheduler/ → cron jobs (grace period processor)
